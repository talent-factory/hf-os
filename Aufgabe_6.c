...
void print_file_metadata(const char* filename) {
    ...
    // Formatierung und Ausgabe des letzten Zugriffszeitpunkts
    ...
    // Formatierung und Ausgabe des letzten Änderungszeitpunkts
    char modify_time_str[30];
    strftime(modify_time_str, sizeof(modify_time_str), "%Y-%m-%d %H:%M:%S", 
...
int main() {
    ...
    fprintf(file, "Dies sollte die Metadaten der Datei ändern.\n");
    fclose(file);
