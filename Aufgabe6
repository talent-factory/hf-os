...
void print_file_metadata(const char* filename) {
    ...
    char time_str[30];
    strftime(time_str, sizeof(time_str), "%Y-%m-%d %H:%M:%S", localtime(&file_info.st_atime)); // → Ergänzung
    printf("Letzter Zugriff: %s\n", time_str);
    // - Letzte Änderung (als formatiertes Datum)
    strftime(time_str, sizeof(time_str), "%Y-%m-%d %H:%M:%S", localtime(&file_info.st_mtime)); // → Ergänzung
...
int main() {
    ...
    fprintf(file, "Dies ist der zusätzliche Text.\n");
    fclose(file);
