...
void analyze_memory_layout(int* stack_var, int* heap_var, int* static_var) {
    ...
    unsigned long diff_stack_heap = (unsigned long)heap_var - (unsigned long)stack_var;
    unsigned long diff_stack_static = (unsigned long)static_var - (unsigned long)stack_var;
    unsigned long diff_heap_static = (unsigned long)heap_var - (unsigned long)static_var;

    printf("\nAdressdifferenzen (in Bytes):\n");
    printf("Heap - Stack:      %ld\n", diff_stack_heap);
    printf("Static - Stack:    %ld\n", diff_stack_static);
    ...
    *heap_variable = 300;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (leaked_memory != NULL) {
            leaked_memory[0] = i; // kleine Nutzung, aber kein free()
        }
    }
